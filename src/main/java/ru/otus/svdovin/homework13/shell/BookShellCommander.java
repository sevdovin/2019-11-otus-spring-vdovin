...
import ru.otus.svdovin.homework13.service.*;
...
public class BookShellCommander {
    ...
    public void createBook(@ShellOption String bookname, @ShellOption String authorid, @ShellOption String genreid) {
        if (!authorProvider.getAuthorById(authorid).isPresent()) {
            ...
            String newId = bookProvider.createBook(Book.builder().bookName(bookname).genre(genre).authors(listAuthor).build());
    ...
    public void updateBook(@ShellOption String id, @ShellOption String newname) {
        if (!bookProvider.existsById(id)) {
            messageService.printMessageByKey(MSG_KEY_BOOK_ID_NOT_EXISTS);
        } else {
            Book book = bookProvider.getBookById(id).orElse(null);
    ...
    public void getBooksByAuthorId(@ShellOption String authorid) {
        if (!authorProvider.getAuthorById(authorid).isPresent()) {
            messageService.printMessageByKey(MSG_KEY_AUTHOR_ID_NOT_EXISTS);
        } else {
    ...
    public void getBooksByGenreId(@ShellOption String genreid) {
