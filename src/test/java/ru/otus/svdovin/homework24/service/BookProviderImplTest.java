...
class BookProviderImplTest {
    ...
    public void init() throws IOException {
        when(aclSidRepository.findBySid("ROLE_ADMIN")).thenReturn(Optional.of(AclSid.builder().id(Long.valueOf(1)).sid("ROLE_ADMIN").principal(false).build()));
        when(aclSidRepository.findBySid("ROLE_USER")).thenReturn(Optional.of(AclSid.builder().id(Long.valueOf(2)).sid("ROLE_USER").principal(false).build()));
        when(aclSidRepository.findBySid("ROLE_ROMAN")).thenReturn(Optional.of(AclSid.builder().id(Long.valueOf(3)).sid("ROLE_ROMAN").principal(false).build()));
        when(aclSidRepository.findBySid("ROLE_COMEDY")).thenReturn(Optional.of(AclSid.builder().id(Long.valueOf(4)).sid("ROLE_COMEDY").principal(false).build()));
    }
    ...
    void shouldReturnExpectedBookById() {
        Book book = new Book(NEW_BOOK_ID, NEW_BOOK_NAME,
                new Genre(TEST_GENRE_ID, TEST_GENRE_NAME));
        when(bookRepository.findById(NEW_BOOK_ID)).thenReturn(Optional.of(book));
    ...
    void shouldReturnExpectedBookByName() {
        ...
        list.add(book);
        when(bookRepository.findByBookName(NEW_BOOK_NAME)).thenReturn(list);
    ...
    void shouldReturnExpectedBookByAuthorId() {
        ...
        List<Book> list = new ArrayList<>();
        list.add(book);
        when(bookRepository.findByAuthorsAuthorId(TEST_AUTHOR_ID)).thenReturn(list);
    ...
    void shouldReturnExpectedBookByGenreId() {
        ...
        List<Book> list = new ArrayList<>();
        list.add(book);
        when(bookRepository.findByGenreGenreId(TEST_GENRE_ID)).thenReturn(list);
    ...
    void shouldCheckExistById() {
        ...
        when(bookRepository.existsById(TEST_BOOK_ID_NOT_EXISTS)).thenReturn(false);
    ...
    void shouldCheckExistByName() {
        when(bookRepository.existsByBookName(TEST_BOOK_NAME)).thenReturn(true);
        when(bookRepository.existsByBookName(TEST_BOOK_NAME_NOT_EXISTS)).thenReturn(false);
